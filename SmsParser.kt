...
object SmsParser {
    ...
    private val DEFAULT_CREDIT = listOf(
        "credited","received","credit","deposited","added","refund","cashback","salary"
    )
    ...
    private fun amountPatterns(currency: String) = listOf(
        ...
        Regex("""([0-9,]+(?:\.[0-9]{1,3})?)\s*(?:${Regex.escape(currency)}|QAR|SAR|AED)""", RegexOption.IGNORE_CASE),
    ...
    private val MERCHANT_PATTERNS = listOf(
        Regex("""(?:at|to|At|To|merchant[:\s]+)\s+([A-Za-z0-9 \-&'.\/]+?)(?:\s+on\s+|\s+ref|\s+txn|\s+for|\s*\.|,|\s{2,}|$)"""),
    ...
    // Public so ViewModel can inspect matched keywords
    val CATEGORY_KEYWORDS: Map<ExpenseCategory, List<String>> = mapOf(
        ExpenseCategory.FOOD to listOf(
            ...
            "talabat","starbucks","tim hortons","hardees","popeyes","wendys",
            "dominos","papa johns","little caesars","careem food","noon food"
        ),
        ExpenseCategory.GROCERY to listOf(
            "lulu","carrefour","safari","al meera","monoprix","spinneys",
            "waitrose","family food","geant","hypermarket","supermarket",
            "grocery","fresh","co-op","spar","grand mart"
        ),
    ...
    /** Returns keyword list for a given category — used by ViewModel for debug view */
    fun getCategoryKeywords(category: ExpenseCategory): List<String> =
        CATEGORY_KEYWORDS[category] ?: emptyList()
    ...
    fun parse(
        ...
        val amount   = extractAmount(sms, amountPatterns(config.currencySymbol)) ?: return null
        ...
        return ParsedTransaction(amount, merchant, balance, category, receivedAt, sms, computeHash(sms), sender)
    }
